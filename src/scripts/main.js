...
import { CONFIG, ASSETS } from '../config/content.js';
...
function initSurpriseBox() {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    box.innerHTML = `
      ...
      <div class="s-label">Surprise ${i+1}</div>
    `;
    box.addEventListener('click', function () { openSurprise(parseInt(this.dataset.idx)); });
    container.appendChild(box);
  }
  gsap.fromTo('.s-card', { opacity: 0, y: 20, scale: 0.9, force3D: true }, {
    opacity: 1, y: 0, scale: 1, duration: 0.4, stagger: 0.06, ease: 'back.out(1.7)', force3D: true
  });
}
...
function openSurprise() {
  ...
  document.getElementById('surpriseIcon').textContent = item.icon + ' ' + randomPhoto;
...
function initDailyNotes() {
  ...
  notes.forEach((note, idx) => {
    ...
    div.innerHTML = `
      ...
      <div class="note-preview">${isUnlocked ? note.substring(0, 30) + '…' : '🔒'}</div>
      ${!isUnlocked ? '<div class="lock-icon-small">🔒</div>' : ''}
    `;
...
function loadWishes() {
  try { const saved = localStorage.getItem('birthdayWishes'); if (saved) wishList = JSON.parse(saved); } catch (_) { wishList = []; }
  renderWishes();
}
...
function renderWishes() {
  ...
  wishList.forEach((w, idx) => {
    ...
    div.innerHTML = `
      ...
      <button class="wish-delete" data-idx="${idx}">✕</button>
    `;
  ...
  if (wishList.length === 0) {
    list.innerHTML = '<div style="text-align:center;padding:30px;color:#b08bb0;font-family:var(--font-hand);font-size:18px;">✨ Make your first wish… ✨</div>';
  }
...
function initStories() {
  ...
  gsap.fromTo('.story-frame', { opacity: 0, y: 30, scale: 0.95, force3D: true }, {
    opacity: 1, y: 0, scale: 1, duration: 0.5, stagger: 0.07, ease: 'back.out(1.4)', force3D: true,
    scrollTrigger: { trigger: '#stories-scroll', start: 'top 85%', once: true }
  });
}
...
function typeQuote(el, text) {
  ...
  text.split('').forEach((ch, i) => {
    ...
    gsap.to(span, { opacity: 1, duration: 0.02, delay: i * 0.035, onStart: function () { span.classList.add('show'); } });
  });
}
...
function initAlbum() {
  ...
  let row = document.createElement('div'); row.className = 'album-row';
  let cnt = 0;
  data.forEach((item, idx) => {
    const frame = document.createElement('div');
    frame.className = 'album-frame ' + item.size + ' ' + (item.tilt || '');
    ...
    const cap = document.createElement('div'); cap.className = 'frame-caption'; cap.textContent = item.caption;
    frame.appendChild(cap);
    const lb = document.createElement('div'); lb.className = 'frame-label'; lb.textContent = item.label;
  ...
  gsap.fromTo('.album-frame', { opacity: 0, y: 20, scale: 0.9, force3D: true }, {
    opacity: 1, y: 0, scale: 1, duration: 0.4, stagger: 0.04, ease: 'back.out(1.7)', force3D: true,
    scrollTrigger: { trigger: '#album-wall', start: 'top 85%', once: true }
  });
}
...
function initJourney() {
  ...
  data.forEach((item, idx) => {
    ...
    tile.className = 'journey-tile' + (idx === 0 ? ' active-tile' : '');
    tile.dataset.idx = idx;
    tile.style.transform = 'translateX(' + (idx % 2 === 0 ? 0 : 8) + 'px)';
    const head = document.createElement('div'); head.className = 'tile-head';
    const icon = document.createElement('span'); icon.className = 'tile-icon'; icon.textContent = item.icon;
    head.appendChild(icon);
    const info = document.createElement('div'); info.className = 'tile-info';
    const title = document.createElement('div'); title.className = 'tile-title'; title.textContent = item.title;
    info.appendChild(title);
    const date = document.createElement('div'); date.className = 'tile-date'; date.textContent = item.date;
    info.appendChild(date);
    head.appendChild(info);
    tile.appendChild(head);
    const q = document.createElement('div'); q.className = 'tile-quote'; q.textContent = '"' + item.quote + '"';
    tile.appendChild(q);
    const mg = document.createElement('div'); mg.className = 'tile-mini-game'; mg.dataset.gameType = item.gameType;
    mg.innerHTML = '<span style="font-size:13px;opacity:0.6;">✦ interact ✦</span>';
    tile.appendChild(mg);
    path.appendChild(tile);
  });
  const m = document.createElement('div'); m.className = 'journey-marker'; m.id = 'journey-marker'; m.textContent = '💖';
  path.appendChild(m);
  updateJourney(0);
...
function updateJourney(idx) {
  ...
  const nb = document.getElementById('journey-next');
  nb.disabled = idx === CONFIG.journeyMilestones.length - 1;
  nb.textContent = idx === CONFIG.journeyMilestones.length - 1 ? '🎉 Celebrate!' : 'Next Step →';
  initMiniGame(idx);
}

function initMiniGame(idx) {
  const tile = document.querySelector('.journey-tile[data-idx="' + idx + '"]');
  if (!tile) return;
  const mg = tile.querySelector('.tile-mini-game');
  const type = mg.dataset.gameType;
  const item = CONFIG.journeyMilestones[idx];
  mg.innerHTML = '';
  mg.style.gap = '8px';
  switch (type) {
    case 'pop':
      mg.innerHTML = '<div style="display:flex;gap:8px;flex-wrap:wrap;justify-content:center;"><span class="mg-balloon" style="font-size:32px;cursor:pointer;">🎈</span><span class="mg-balloon" style="font-size:32px;cursor:pointer;">🎈</span><span class="mg-balloon" style="font-size:32px;cursor:pointer;">🎈</span></div><div class="mg-text-reveal">💕 ' + item.quote + '</div>';
      mg.querySelectorAll('.mg-balloon').forEach(el => {
        el.addEventListener('click', function () { gsap.to(this, { scale: 1.5, opacity: 0, duration: 0.3, ease: 'back.in(2)', onComplete: function () { this.remove(); } }); mg.querySelector('.mg-text-reveal').classList.add('show'); launchConfetti(30, { spread: 60, origin: { x: (tile.getBoundingClientRect().left + tile.getBoundingClientRect().width / 2) / window.innerWidth, y: tile.getBoundingClientRect().top / window.innerHeight } }); });
      });
      break;
    case 'hold': {
      const star = document.createElement('span'); star.textContent = '⭐'; star.style.cssText = 'font-size:40px;cursor:pointer;';
      mg.appendChild(star);
      const rv = document.createElement('div'); rv.className = 'mg-text-reveal'; rv.textContent = '✨ ' + item.quote;
      mg.appendChild(rv);
      let ht;
      ...
      star.addEventListener('pointerup', function () { clearTimeout(ht); });
      star.addEventListener('pointerleave', function () { clearTimeout(ht); });
      break;
    }
    case 'connect':
      mg.innerHTML = '<div style="display:flex;gap:12px;align-items:center;"><span class="mg-heart" style="font-size:28px;cursor:pointer;">💗</span><span style="font-size:16px;opacity:0.3;">+</span><span class="mg-heart" style="font-size:28px;cursor:pointer;">💗</span></div><div class="mg-text-reveal">💞 ' + item.quote + '</div>';
      let cn = 0;
      mg.querySelectorAll('.mg-heart').forEach(el => {
        el.addEventListener('click', function () { cn++; gsap.to(this, { scale: 1.5, duration: 0.3, ease: 'back.out(2)', yoyo: true, repeat: 1 }); if (cn >= 2) { mg.querySelector('.mg-text-reveal').classList.add('show'); launchConfetti(50, { spread: 120, origin: { x: (tile.getBoundingClientRect().left + tile.getBoundingClientRect().width / 2) / window.innerWidth, y: tile.getBoundingClientRect().top / window.innerHeight } }); } });
      });
      break;
    case 'reveal':
      mg.innerHTML = '<button class="mg-btn">🎂 Open Gift</button><div class="mg-text-reveal">🎉 ' + item.quote + '</div>';
      mg.querySelector('.mg-btn').addEventListener('click', function () { mg.querySelector('.mg-text-reveal').classList.add('show'); launchConfetti(80, { spread: 180, origin: { x: 0.5, y: 0.4 } }); gsap.to(this, { scale: 0.8, opacity: 0, duration: 0.3 }); });
      break;
    case 'drag':
      mg.innerHTML = '<div style="font-size:13px;opacity:0.6;text-align:center;">☁️ drag the cloud away ☁️</div><div class="mg-cloud" style="font-size:48px;cursor:grab;user-select:none;touch-action:none;">☁️</div><div class="mg-text-reveal">☀️ ' + item.quote + '</div>';
      const cloud = mg.querySelector('.mg-cloud');
      let dsx, dsy, drg = false;
      cloud.addEventListener('pointerdown', function (e) { drg = true; dsx = e.clientX; dsy = e.clientY; this.setPointerCapture(e.pointerId); });
      cloud.addEventListener('pointermove', function (e) { if (!drg) return; const dx = e.clientX - dsx, dy = e.clientY - dsy; gsap.to(this, { x: dx, y: dy, opacity: 1 - Math.abs(dx) / 200, duration: 0.1, ease: 'none' }); if (Math.abs(dx) > 80) { drg = false; gsap.to(this, { opacity: 0, scale: 0.5, duration: 0.3, onComplete: function () { this.remove(); } }); mg.querySelector('.mg-text-reveal').classList.add('show'); launchConfetti(40, { spread: 90, origin: { x: (tile.getBoundingClientRect().left + tile.getBoundingClientRect().width / 2) / window.innerWidth, y: tile.getBoundingClientRect().top / window.innerHeight } }); } });
      cloud.addEventListener('pointerup', function () { drg = false; });
      cloud.addEventListener('pointercancel', function () { drg = false; });
      break;
  }
}
...
function initPostcard() {
  ...
  const deco = document.getElementById('postcard-deco-frames');
  deco.innerHTML = '';
  data.forEach((item, idx) => {
    const f = document.createElement('div'); f.className = 'mini-frame'; f.style.setProperty('--rot', item.rot + 'deg'); f.textContent = item.emoji;
    const c = document.createElement('span'); c.className = 'mini-caption'; c.textContent = item.caption;
    f.appendChild(c);
    f.addEventListener('click', function (e) { e.stopPropagation(); switchPostcard(idx); });
    deco.appendChild(f);
  });
  switchPostcard(0);
  document.getElementById('postcard-main').addEventListener('click', function () {
    ...
    const ct = document.getElementById('letter-text');
    const tx = data[currentPostcardIdx].letter;
    ct.innerHTML = '';
    tx.split('').forEach((ch, i) => {
      if (ch === '\n') { ct.appendChild(document.createElement('br')); return; }
      const s = document.createElement('span'); s.className = 'char'; s.textContent = ch === ' ' ? '\u00A0' : ch;
      ct.appendChild(s);
      gsap.set(s, { opacity: 0 });
      gsap.to(s, { opacity: 1, duration: 0.02, delay: 0.3 + i * 0.025, onStart: function () { s.classList.add('show'); } });
    });
    for (let i = 0; i < 12; i++) {
      gsap.to({}, { delay: i * 0.15, onStart: function () {
        const h = document.createElement('span');
        h.textContent = ['💕', '♥', '💗', '✨'][Math.floor(Math.random() * 4)];
        gsap.set(h, { position: 'absolute', pointerEvents: 'none', fontSize: 16, left: 20 + Math.random() * 60 + '%', top: 20 + Math.random() * 60 + '%', zIndex: 6 });
        this.appendChild(h);
        gsap.to(h, { y: -60 - Math.random() * 40, x: -20 + Math.random() * 40, opacity: 0, scale: 0.3, duration: 1.5 + Math.random(), ease: 'power3.out', onComplete: function () { h.remove(); } });
      }.bind(this) });
    }
...
function switchPostcard(idx) {
  if (idx === currentPostcardIdx && isLetterRevealed) {
    const m = document.getElementById('postcard-main');
    if (m.classList.contains('flipped')) { m.classList.remove('flipped'); isLetterRevealed = false; }
    return;
  }
  currentPostcardIdx = idx;
  const d = CONFIG.postcards[idx];
  document.getElementById('postcard-icon').textContent = d.frontIcon;
  document.getElementById('postcard-label').textContent = d.frontLabel;
  const m = document.getElementById('postcard-main');
...
function initSecret() {
  ...
  secretInit = true;
  const tu = function () {
    const v = document.getElementById('lock-input').value.trim().toLowerCase();
    if (v === CONFIG.password) { unlockSecret(); } else { document.getElementById('lock-hint').classList.add('show'); gsap.fromTo('#lock-hint', { opacity: 0 }, { opacity: 1, duration: 0.4 }); gsap.fromTo('#secret-lock .lock-icon', { scale: 1 }, { scale: 1.2, duration: 0.2, ease: 'back.out(2)', yoyo: true, repeat: 1 }); }
  };
...
function unlockSecret() {
  ...
  gsap.to('#secret-lock', { opacity: 0, scale: 0.9, duration: 0.4, ease: 'power2.in', onComplete: function () {
    document.getElementById('secret-lock').style.display = 'none';
    document.getElementById('secret-gallery').classList.add('unlocked');
    gsap.fromTo('#secret-gallery', { opacity: 0 }, { opacity: 1, duration: 0.5, ease: 'power2.out' });
    populateSecretGallery();
    initScratch();
    launchConfetti(120, { spread: 360, origin: { x: 0.5, y: 0.4 } });
    launchConfetti(80, { spread: 180, origin: { x: 0.3, y: 0.3 } });
    launchConfetti(80, { spread: 180, origin: { x: 0.7, y: 0.3 } });
  } });
}

function populateSecretGallery() {
  const s = document.getElementById('gallery-scatter');
  s.innerHTML = '';
  const e = ['📸', '🌅', '🌸', '☕', '🌙', '🎵', '✨', '💕', '🌟', '🎂', '💫', '🌈'];
  const r = [-3, 4, -2, 5, -4, 3, -1, 6, -5, 2, -3, 4];
  for (let i = 0; i < 12; i++) {
    const f = document.createElement('div'); f.className = 'scatter-frame';
    f.style.setProperty('--rot', (r[i % r.length] + Math.random() * 2 - 1) + 'deg');
    const fl = document.createElement('div'); fl.className = 'frame-fill'; fl.textContent = e[i % e.length];
    f.appendChild(fl);
...
function initScratch() {
  ...
  scratchW = r.width; scratchH = r.height;
  c.width = scratchW * dpr; c.height = scratchH * dpr;
  c.style.width = scratchW + 'px'; c.style.height = scratchH + 'px';
  scratchCtx = c.getContext('2d');
  scratchCtx.scale(dpr, dpr);
  scratchCtx.fillStyle = '#e8dce0';
  scratchCtx.fillRect(0, 0, scratchW, scratchH);
  scratchCtx.fillStyle = 'rgba(200,180,185,0.2)';
  for (let i = 0; i < 40; i++) { scratchCtx.beginPath(); scratchCtx.arc(Math.random() * scratchW, Math.random() * scratchH, 2 + Math.random() * 6, 0, Math.PI * 2); scratchCtx.fill(); }
  scratchCtx.fillStyle = 'rgba(255,107,157,0.08)';
  scratchCtx.font = '40px serif'; scratchCtx.textAlign = 'center';
  scratchCtx.fillText('✨', scratchW / 2, scratchH / 2 + 10);
  scratchPercent = 0; scratchRevealed = false;
  document.getElementById('scratch-reveal').classList.remove('show');
  document.getElementById('scratch-progress').textContent = '0% revealed';
  const pr = document.getElementById('scratch-progress');
  const gp = function (e) { const r = c.getBoundingClientRect(); const cx = e.touches ? e.touches[0].clientX : e.clientX, cy = e.touches ? e.touches[0].clientY : e.clientY; return { x: cx - r.left, y: cy - r.top }; };
  const sc = function (pos) {
    if (!scratchCtx) return;
    scratchCtx.globalCompositeOperation = 'destination-out'; scratchCtx.beginPath(); scratchCtx.arc(pos.x, pos.y, 22, 0, Math.PI * 2); scratchCtx.fill(); scratchCtx.globalCompositeOperation = 'source-over';
    const id = scratchCtx.getImageData(0, 0, scratchW, scratchH); let tr = 0; const t = id.data.length / 4;
    for (let i = 3; i < id.data.length; i += 4) { if (id.data[i] < 128) tr++; }
    const pct = Math.round((tr / t) * 100); scratchPercent = pct; pr.textContent = pct + '% revealed';
    if (pct > 0 && pct % 10 === 0) { gsap.fromTo(pr, { scale: 1.2 }, { scale: 1, duration: 0.3, ease: 'back.out(2)' }); }
    if (pct >= 70 && !scratchRevealed) {
      scratchRevealed = true; document.getElementById('scratch-reveal').classList.add('show');
      launchConfetti(100, { spread: 360, origin: { x: 0.5, y: 0.5 } });
      launchConfetti(80, { spread: 180, origin: { x: 0.3, y: 0.4 } });
      launchConfetti(80, { spread: 180, origin: { x: 0.7, y: 0.4 } });
      setTimeout(() => { showEnding(); }, 2000);
    }
  };
  c.onpointerdown = function (e) { isScratching = true; sc(gp(e)); };
  c.onpointermove = function (e) { if (isScratching) sc(gp(e)); };
  c.onpointerup = function () { isScratching = false; };
  c.onpointercancel = function () { isScratching = false; };
  c.ontouchstart = function (e) { e.preventDefault(); isScratching = true; sc(gp(e)); };
  c.ontouchmove = function (e) { e.preventDefault(); if (isScratching) sc(gp(e)); };
  c.ontouchend = function () { isScratching = false; };
}
...
function showEnding() {
  ...
  launchConfetti(150, { spread: 360, origin: { x: 0.5, y: 0.3 } });
  launchConfetti(100, { spread: 180, origin: { x: 0.2, y: 0.4 } });
  launchConfetti(100, { spread: 180, origin: { x: 0.8, y: 0.4 } });
...
document.getElementById('musicPlayer').addEventListener('click', function () {
  ...
  alert('🎵 Now playing: ' + (track ? track.label : 'Unknown') + '\n(Add real audio files to ' + (track && track.src ? track.src : 'public/music/') + ')');
});
